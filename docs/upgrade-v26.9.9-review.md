...
保留此前审核通过的 TLS 快照补丁。核心已推送至 `HoshinoNeko/Xray-core` 的
`fix/tls-snapshot-v26.9.9` 分支，固定提交为 `a2192dfad2be562ff9b4fa4d4478aa454b6065e1`。
`go.mod` 使用该提交的远程 Go 伪版本，不再依赖本地路径。伪版本中的旧数字前缀
来自 fork 可见的语义版本标签；源码基础确实是上述 v26.9.9 提交，并非回退版本。
本次仅修复远程依赖交付；下述 UDP/udphop 问题仍未修复，不能据此视为生产验收通过。
