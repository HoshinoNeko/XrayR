...
`nolego` 标签与 minimal 等价；两者均不指定才是完整版。go.mod/go.sum 仍保留完整版依赖，构建前的
...
按用户后续确认，正式 Release 完整版及完整版镜像执行 UPX；minimal/nolego 不加壳。
发布 Release 或手动触发 Build and Release 工作流（workflow_dispatch，可选择分支）时，
额外生成 Actions-only 的 UPX 副本（仅完整版），
...
不强制加壳、不以未压缩文件冒充 UPX 版。若平台不支持，Artifact 只包含报告。
...
实际代理流量和进程管理兼容性验证。上表为早期测量，minimal 不再发布加壳副本。
最新运行时和镜像设置见 [内存优化说明](memory-tuning.md)。
